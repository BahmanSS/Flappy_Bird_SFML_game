...
PipeManager::PipeManager(Bird &bird_, unsigned int width_, unsigned int height_)
    : width(width_), height(height_), bird(bird_) {
    pipe_1 = std::make_unique<Pipe>(static_cast<unsigned int>(std::rand() % 101),
                                    width, height);
}
bool PipeManager::update(float dt) {
    BirdPipeCollider collider;
    bool pipeCol1 = collider.checkCollision(bird, *pipe_1);
    bool pipeCol2 = false;
    bool pipeCol3 = false;
    if (pipe_2) {
        pipeCol2 = collider.checkCollision(bird, *pipe_2);
    }
    if (pipe_3) {
        pipeCol3 = collider.checkCollision(bird, *pipe_3);
    }
    bool allCollision = false;
    if (pipeCol1 || pipeCol2 || pipeCol3) {
        allCollision = true;
    }
...
bool PipeManager::checkBirdPassedPipes() {
    BirdPipeCollider collider;
...
bool PipeManager::checkCollision() {
    BirdPipeCollider collider;
